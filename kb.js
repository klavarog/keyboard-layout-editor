  ...
  kbApp.controller('kbCtrl', ['$scope','$http','$location','$timeout', '$sce', '$sanitize', '$modal', '$cookies', '$confirm', '$q', function($scope, $http, $location, $timeout, $sce, $sanitize, $modal, $cookies, $confirm, $q) {
    ...
    $scope.removeLegendsButtons = [
      { label: "All",
        re: /.*/,
        decals: true,
        tooltip: "Remove all the legends including decals." },
      { label: "Alphas",
        re: /^[A-Za-zа-яёА-ЯЁ]$/,
        tooltip: "Remove one-letter legends." },
      { label: "Numbers",
        re: /^[0-9]*$/,
        tooltip: "Remove numeric legends." },
      { label: "Non-alphanumeric",
        re: RegExp('^[^a-zA-Zа-яА-ЯёЁ0-9]$', 'i'),
        tooltip: "Remove non-alphanumeric legends." },
      { label: "F-keys",
        re: /^F\d\d?$/,
        tooltip: "Remove F-key legends (F1, F2...)." },
      { label: "Specials",
        re: /<.*>/,
        tooltip: "Remove special legends, like FontAwesome, WebFont, images, etc, and anything between them in the same slot." },
      { label: "Others", 
        re: /^.*$/,
        tooltip: "Remove all legends except from decals." },
      { label: "Decals",
        re: /.*/,
        decals: true,
